...
class HomeController extends Controller
{
    ...
    {
        // Real-time Aggregated News (NEW - World-class features)
        $liveTickerNews = Cache::remember('live_ticker_news', 60, function() {
            return AggregatedNews::with('source')
                ->where('is_breaking', true)
                ->orWhere('published_at', '>=', now()->subHours(6))
                ->orderBy('published_at', 'DESC')
                ->take(20)
                ->get();
        });

        $trendingNews = Cache::remember('trending_news', 120, function() {
            return AggregatedNews::with('source')
                ->where('is_trending', true)
                ->orWhere('engagement_score', '>', 50)
        ...
        $breakingAggregated = Cache::remember('breaking_aggregated', 30, function() {
            return AggregatedNews::with('source')
                ->where('is_breaking', true)
                ->orderBy('published_at', 'DESC')
                ->take(5)
        ...
        $latestAggregated = Cache::remember('latest_aggregated', 60, function() {
            return AggregatedNews::with('source')
                ->orderBy('published_at', 'DESC')
                ->take(30)
                ->get();
        ...
        $newsBySource = Cache::remember('news_by_source', 120, function() {
            $sources = NewsSource::active()->byPriority()->take(6)->get();
            $result = [];
            foreach ($sources as $source) {
                $result[$source->slug] = [
                    'source' => $source,
                    'news' => AggregatedNews::where('news_source_id', $source->id)
                        ->orderBy('published_at', 'DESC')
                        ->take(5)
                        ->get()
                ];
        ...
        $trendingTopics = Cache::remember('trending_topics', 300, function() {
        ...
        $newsSources = NewsSource::active()->byPriority()->get();

        // Original News Portal Data
